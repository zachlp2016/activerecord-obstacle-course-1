...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:orders).joins(:items).where('item_id = 9').distinct.order(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    items_for_user_3_third_order = User.joins(:orders).joins(:items).find(3).orders[2].items.pluck(:name)
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = User.joins(:orders).find(3).orders.average(:amount)
    # ------------------------------------------------------------
